...
import { postCategory, postPromo } from '../../stores/actionCreator';
...
export default function AddPromoCode() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await dispatch(postPromo(data));
      setData(initialState); // Reset the form data
      document.getElementById('my_promo_code').close();
  ...
  return (
    <div>
      <button
        ...
        onClick={() => document.getElementById('my_promo_code').showModal()}
      >
        Add Promo
      </button>
      <dialog id="my_promo_code" className="modal">
        <div className="modal-box">
          <form onSubmit={handleSubmit}>
            <button
              type="button"
              className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2"
              onClick={() => document.getElementById('my_promo_code').close()}
            >
              ✕
            </button>
